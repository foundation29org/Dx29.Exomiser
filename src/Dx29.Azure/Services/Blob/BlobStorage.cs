...
namespace Dx29.Services
{
    public partial class BlobStorage
    {
        ...
        public async Task<string> CopyBlobFromUriAsync(string blobContainerName, string path, Uri sourceUri)
        {
            var containerClient = BlobServiceClient.GetBlobContainerClient(blobContainerName);
            var targetClient = containerClient.GetBlockBlobClient(path);
            var operation = await targetClient.StartCopyFromUriAsync(sourceUri);
            return operation.Id;
        }

        public async Task<bool> WaitForCopyOperationAsync(string blobContainerName, string path, string operationId, int timeout = 5 * 60)
        {
            var containerClient = BlobServiceClient.GetBlobContainerClient(blobContainerName);
            var blobClient = containerClient.GetBlockBlobClient(path);
            var operation = new CopyFromUriOperation(operationId, blobClient);
        ...
        public async Task<long> SyncCopyFromUriAsync(string blobContainerName, string path, Uri sourceUri)
        {
            var http = new HttpClient { BaseAddress = sourceUri };
            using (var stream = await http.DownloadAsync(""))
            {
                await UploadStreamAsync(blobContainerName, path, stream);
